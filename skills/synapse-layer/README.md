...
This skill integrates [Synapse Layer](https://synapselayer.org) — an AES-256-GCM encrypted, cross-session memory system — into Hermes Agent. All memories are encrypted at rest, pass through a 3-layer Cognitive Security Pipeline (PII redaction, differential privacy, intent validation), and are retrievable from any channel (Telegram, WhatsApp, CLI, Discord).
...
- **AES-256-GCM Encryption at Rest**: Memories are encrypted with per-operation random IV and GCM authentication tag validated on every read.
...
4. **Secure Agent Handover** — Encrypted cross-agent context transfer (V2)
