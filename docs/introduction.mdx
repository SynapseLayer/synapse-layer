...
memory = Synapse(token="sk_connect_YOUR_TOKEN", agent_id="my-agent")

# 1. Agents save context automatically
memory.store("User prefers absolute security and cross-model continuity.")

# 2. Recall is deterministic and explainable
info = memory.recall("user focus")
# Result: "User prefers absolute security and cross-model continuity."
```
...
<CardGroup cols={2}>
  ...
  <Card title="MCP Server" icon="plug" href="https://docs.synapselayer.org/docs/mcp-server">
    13 tools ready for Claude Desktop, Cursor, Codex, and custom agents.
  </Card>
</CardGroup>
...
| No cross-model context | Cross-model continuity with JWT-signed transfer |
